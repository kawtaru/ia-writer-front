...
const FormLayouts = ({}) => {
  ...
  const checkText = (text) => {
    const regex = `\n  Correct the following text : "${text}" and give me result with corrected words wrapped by <b style='color: red;'> tag \n`
     setLoading(true)
    ...
    <DatePickerWrapper>
      <Grid container spacing={6}>
        <Grid item xs={12} md={6}>
         <FormLayoutsGrammarChecker loading={loading} onCheck={(text) => checkText(text)} />
        </Grid>
        <Grid item xs={12} md={6}>
